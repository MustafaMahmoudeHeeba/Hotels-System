...
const API_AUTH = "";
const API_HOTEL = "";
const API_NOTIFICATIONS = "http://localhost:8083";
...
function LiveEventsPage() {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      const res = await fetch(`${API_NOTIFICATIONS}/notifications`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      setEvents(Array.isArray(data) ? data : []);
      setError(null);
    } catch (e) {
      setError("Unable to connect to event service");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const dotColor = error ? COLORS.danger : COLORS.success;

  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <h2 style={{ fontFamily: font, fontSize: 36, color: COLORS.text, margin: "0 0 4px", fontWeight: 400 }}>Live Events</h2>
        <p style={{ color: COLORS.textMuted, fontSize: 14, margin: 0 }}>Real-time events from the system</p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontFamily: font, fontSize: 20, color: COLORS.text, fontWeight: 400 }}>Event Stream</span>
            <span style={{
          ...
          <span style={{ fontFamily: fontSans, fontSize: 12, color: COLORS.textMuted }}>
            {error ? "Offline" : `${events.length} event${events.length !== 1 ? "s" : ""}`}
          </span>
        </div>
        ...
        <div style={{
          ...
          {loading ? (
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "20px", justifyContent: "center" }}>
              <Spinner />
              <span style={{ fontFamily: fontSans, fontSize: 14, color: COLORS.textMuted }}>Connecting to event stream...</span>
            </div>
          ) : error ? (
            <div style={{
              background: "#e05c5c12", border: `1px solid ${COLORS.danger}33`,
              borderRadius: 8, padding: "16px", textAlign: "center",
            }}>
              <span style={{ fontFamily: fontSans, fontSize: 14, color: COLORS.danger }}>⚠ {error}</span>
            </div>
          ) : events.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              <span style={{ fontSize: 48, opacity: 0.3 }}>📡</span>
              <p style={{ fontFamily: fontSans, fontSize: 14, color: COLORS.textMuted, marginTop: 12 }}>
                No events yet. Waiting for activity...
              </p>
            </div>
          ) : (
            events.map((n, i) => {
              const msg = typeof n === "string" ? n : (n.message || n.content || n.event || JSON.stringify(n));
              const ts = n.timestamp || n.time || n.createdAt || null;
              return (
                <div key={i} style={{
                  display: "flex", justifyContent: "space-between", alignItems: "flex-start",
                  background: COLORS.bgInput, borderRadius: 8, padding: "12px 16px", gap: 16,
                  borderLeft: `3px solid ${COLORS.accent}`,
                }}>
                  <span style={{ fontFamily: fontSans, fontSize: 13, color: COLORS.text, lineHeight: 1.5, flex: 1 }}>
                    <span style={{ color: COLORS.info, marginRight: 8 }}>▸</span>{msg}
                  </span>
                  {ts && (
                    <span style={{ fontFamily: fontSans, fontSize: 11, color: COLORS.textMuted, whiteSpace: "nowrap" }}>
...
function ReservationsPage({ token, showToast }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([
      request(`/api/reservations`, "GET", undefined, token),
      request(`/hotels`, "GET", undefined, token),
      request(`/rooms`, "GET", undefined, token),
    ]).then(([res, h, r]) => {
      setReservations(res || []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await request(`/api/reservations`, "POST", {
  ...
  const handleDelete = async (id) => {
    try {
      await request(`/api/reservations/${id}`, "DELETE", undefined, token);
