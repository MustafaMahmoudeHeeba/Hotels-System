package com.aydin.notificationservice.config;
...
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, ReservationEvent> consumerFactory() {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "notification-group");  // نفس المجموعة
    ...
    @Bean
    public ConsumerFactory<String, ReservationEvent> cancelledConsumerFactory() {
        JsonDeserializer<ReservationEvent> deserializer = new JsonDeserializer<>(ReservationEvent.class);
        deserializer.addTrustedPackages("*");

        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "kafka:9092");
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "notification-cancelled-group");  // مجموعة منفصلة
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, deserializer);
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, ReservationEvent> 
            cancelledKafkaListenerContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, ReservationEvent> factory = 
            new ConcurrentKafkaListenerContainerFactory<>();
